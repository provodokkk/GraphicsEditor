...
void client_code()
{
	...
	Component* leaf = new Circle(20, 110, 110); // branch_1
	Component* leaf_1 = new Circle(50, 30, 30);

	Component* leaf_2 = new Circle(40, 175, 175); // branch_2
	Component* leaf_4 = new Rectangle(40, 20, 260, 360);
	Component* leaf_6 = new Circle(40, 220, 50);

	Component* leaf_3 = new Rectangle(40, 20, 60, 170); // branch_3
	Component* leaf_5 = new Circle(40, 50, 275);

	Component* leaf_7 = leaf_6->copy_elem(); // independent elements
	Component* leaf_8 = new Circle(30, 370, 300);
	
	
	tree->add(branch_2); // tree
	tree->add(branch_3);

	tree->add(leaf_7); // independent elements
	tree->add(leaf_8);
	
	
	branch_1->add(leaf); // branch_1
	branch_1->add(leaf_1);
	
	branch_2->add(leaf_2); // branch_2
	branch_2->add(leaf_4);
	branch_2->add(leaf_6);

	branch_3->add(leaf_3); // branch_3
	branch_3->add(leaf_5);


	Component* branch_4 = branch_3->copy_elem();  // branch_4
	tree->add(branch_4);

	branch_2->add(branch_1);
	branch_1->relocate_elem(branch_3); // relocating composite element branch_3 to branch_1
	...
	RenderWindow window(VideoMode(800, 500), "Laboratory work 5");
	Vector2i position;
	while (window.isOpen())
	{
